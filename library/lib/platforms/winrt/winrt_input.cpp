...
static auto controllers { winrt::single_threaded_vector<winrt::Windows::Gaming::Input::UINavigationController>() };
...
WinRTInputManager::WinRTInputManager(winrt::Windows::UI::Core::CoreWindow coreWindow)
    : coreWindow(coreWindow)
{
    coreWindow.PointerPressed({ this, &WinRTInputManager::OnPointerPressed });
    coreWindow.PointerReleased({ this, &WinRTInputManager::OnPointerReleased });
    coreWindow.PointerMoved({ this, &WinRTInputManager::OnPointerMoved });
    // coreWindow.KeyDown([&](winrt::Windows::Foundation::IInspectable const&, winrt::Windows::UI::Core::KeyEventArgs const& args) {
    //	OnKeyDownOrUp(args, true); });
    // coreWindow.KeyUp([&](winrt::Windows::Foundation::IInspectable const&, winrt::Windows::UI::Core::KeyEventArgs const& args) {
    //	OnKeyDownOrUp(args, false); });
    coreWindow.Dispatcher().AcceleratorKeyActivated([&](winrt::Windows::Foundation::IInspectable const&, winrt::Windows::UI::Core::AcceleratorKeyEventArgs const& args)
        { OnKeyDownOrUp(args, true); });

    /*
    auto smtc = winrt::Windows::Media::SystemMediaTransportControls::GetForCurrentView();
    if (smtc)
    {
        smtc.ButtonPressed({ this, &WinRTInputManager::OnMediaKey });
        smtc.IsEnabled(true);
    }
    */
...
void WinRTInputManager::OnPointerReleased(winrt::Windows::Foundation::IInspectable const&, winrt::Windows::UI::Core::PointerEventArgs const& args)
{
    auto point = args.CurrentPoint();
    if (point.PointerDevice().PointerDeviceType() == winrt::Windows::Devices::Input::PointerDeviceType::Touch)
    {
...
void WinRTInputManager::OnKeyDownOrUp(winrt::Windows::UI::Core::AcceleratorKeyEventArgs const& args, bool keyDown)
{
    ...
    auto code = virtualKeyToBrlsKeyboardScancode(args.VirtualKey());
    if (code == BRLS_KBD_KEY_UNKNOWN)
    {
        return;
    }
    keyboardKeys[code] = keyDown ? BTN_PRESSED : BTN_STICKY;

    auto ctrlState   = (coreWindow.GetKeyState(VirtualKey::Control) & CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down;
    auto shiftlState = (coreWindow.GetKeyState(VirtualKey::Shift) & CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down;
    // auto winState=(coreWindow.GetKeyState(VirtualKey::LeftWindows) & CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down;
    auto altState = args.KeyStatus().IsMenuKeyDown;

    KeyState state {};
    state.key     = code;
    state.pressed = keyDown;
    state.mods |= ctrlState ? BRLS_KBD_MODIFIER_CTRL : 0;
    state.mods |= altState ? BRLS_KBD_MODIFIER_ALT : 0;
    state.mods |= shiftlState ? BRLS_KBD_MODIFIER_SHIFT : 0;
    // state.mods|=winState?BRLS_KBD_MODIFIER_META:0;
    this->getKeyboardKeyStateChanged()->fire(state);
    Application::setActiveEvent(true);
}

void WinRTInputManager::OnMediaKey(winrt::Windows::Media::SystemMediaTransportControls, winrt::Windows::Media::SystemMediaTransportControlsButtonPressedEventArgs const& args)
{
